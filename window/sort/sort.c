  ...
  for (size_t i = 0; i < WINDOW_SORT_OPTIONS_QTY; i++) {
    if (WINDOW_SORT_DEBUG_MODE) {
      log_debug("sort type: #%lu|name:%s|desc:%s|# direction:#%d|key:%s|name:%s|",
                i,
                window_sort_type_names[sorts[i].type],
                window_sort_type_descriptions[sorts[i].type],
                sorts[i].direction,
                window_sort_direction_keys[sorts[i].direction],
                window_sort_direction_names[sorts[i].direction]
                );
    }
    if (strcmp(name, window_sort_type_names[sorts[i].type]) == 0 && strcmp(direction, window_sort_direction_keys[sorts[i].direction]) == 0) {
