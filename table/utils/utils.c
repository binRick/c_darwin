...
#define SPACE_COLUMNS          "ID", "Name", "Key", "Action", "Enabled"
...
int list_installed_apps_table(void *ARGS) {
  ...
  args->limit <= 0 ? DEFAULT_APPS_LIMIT : args->limit;
  ft_table_t         *table;
  struct app_t       *app;
  struct Vector      *apps_v;
  struct table_dur_t durs[TABLE_DUR_TYPES_QTY];
  ...
  {
    durs[TABLE_DUR_TYPE_QUERY_ITEMS].started = timestamp();
    apps_v                                   = tables[TABLE_TYPE_APP]->query_items();
    INIT_TABLE(table, APP_COLUMNS);
  }

  durs[TABLE_DUR_TYPE_FILTER_ROWS].dur = 0;
  for (unsigned long i = 0; i < vector_size(apps_v); i++) {
    durs[TABLE_DUR_TYPE_FILTER_ROWS].started = timestamp();
    BREAK_IF_ROW_LIMIT(table, args);
    durs[TABLE_DUR_TYPE_FILTER_ROWS].dur += timestamp() - durs[TABLE_DUR_TYPE_FILTER_ROWS].started;
    app                                   = VECTOR_ITEM(apps_v, app_t *, i);
    CONTINUE_IF_ROW_SKIP((tables[TABLE_TYPE_FONT]->row_skip(table, i, app, args)));
    tables[TABLE_TYPE_APP]->row(table, i, app);
    tables[TABLE_TYPE_APP]->row_style(table, i, app);
  }
  ft_add_separator(table);
  durs[TABLE_DUR_TYPE_TOTAL].dur = timestamp() - durs[TABLE_DUR_TYPE_TOTAL].started;
  TABLE_SUMMARY("Apps", apps_v, table, durs);
...
int list_hotkeys_table(void *ARGS) {
  ...
  {
    durs[TABLE_DUR_TYPE_QUERY_ITEMS].started = timestamp();
    keys_v                                   = tables[TABLE_TYPE_HOTKEY]->query_items();
    INIT_TABLE(table, HOTKEY_COLUMNS);
  }

  for (unsigned long i = 0; i < vector_size(keys_v); i++) {
    BREAK_IF_ROW_LIMIT(table, args);
    hk = VECTOR_ITEM(keys_v, key_t *, i);
    tables[TABLE_TYPE_HOTKEY]->row(table, i, hk);
    tables[TABLE_TYPE_HOTKEY]->row_style(table, i, hk);
  }
  ft_add_separator(table);
  durs[TABLE_DUR_TYPE_TOTAL].dur = timestamp() - durs[TABLE_DUR_TYPE_TOTAL].started;
  TABLE_SUMMARY("Hotkeys", keys_v, table, durs);
} /* list_hotkeys_table */
...
int list_displays_table(void *ARGS) {
  ...
  durs[TABLE_DUR_TYPE_TOTAL].started = timestamp();
  {
    durs[TABLE_DUR_TYPE_QUERIES].started = timestamp();
    displays_v                           = tables[TABLE_TYPE_DISPLAY]->query_items();
    durs[TABLE_DUR_TYPE_QUERIES].dur     = timestamp() - durs[TABLE_DUR_TYPE_QUERIES].started;
  }
  durs[TABLE_DUR_TYPE_FILTER_ROWS].dur = 0;
  {
    INIT_TABLE(table, DISPLAY_COLUMNS);
  }
  for (size_t i = 0; i < vector_size(displays_v); i++) {
    BREAK_IF_ROW_LIMIT(table, args)
    display = VECTOR_ITEM(displays_v, display_t *, i);
    tables[TABLE_TYPE_DISPLAY]->row(table, i, display);
    tables[TABLE_TYPE_DISPLAY]->row_style(table, i, display);
  }
  TABLE_SUMMARY("Displays", displays_v, table, durs);
} /* list_displays_table */

int list_spaces_table(void *ARGS) {
  struct list_table_t *args     = (struct list_table_t *)ARGS;
  struct Vector       *spaces_v = tables[TABLE_TYPE_SPACE]->query_items();
  ...
  ft_table_t *table;
  {
    INIT_TABLE(table, SPACE_COLUMNS);
  }

  for (size_t i = 0; i < vector_size(spaces_v); i++) {
    BREAK_IF_ROW_LIMIT(table, args)
    space = VECTOR_ITEM(spaces_v, space_t *, i);
    tables[TABLE_TYPE_SPACE]->row(table, i, space);
    tables[TABLE_TYPE_SPACE]->row_style(table, i, space);
  }
  TABLE_SUMMARY("Spaces", spaces_v, table, durs);
} /* list_spaces_table */
