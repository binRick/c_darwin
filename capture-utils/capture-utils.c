...
struct capture_type_t capture_types[CAPTURE_TYPES_QTY + 1] = {
  [CAPTURE_TYPE_SPACE] =   {
    .name                   = "space",
    .get_default_capture_id = ^ size_t (void){ return(get_space_id());                                            },
    .validate_capture_id    = ^ bool (size_t space_id){ return((space_id > 0 && space_id < 10000) ? true : false); },
    .capture                = ^ CGImageRef (size_t space_id){ return(capture_space_id((uint32_t)space_id)); },
  },
  [CAPTURE_TYPE_DISPLAY] = {
    .name                   = "display",
    .get_default_capture_id = ^ size_t (void){ return(get_main_display_id());                                     },
    .validate_capture_id    = ^ bool (size_t display_id){ return((display_id > 0 && display_id < 999999999) ? true : false); },
    .capture                = ^ CGImageRef (size_t display_id){ return(capture_display_id((uint32_t)display_id)); },
  },
  [CAPTURE_TYPE_WINDOW] =  {
    .name                   = "window",
    .get_default_capture_id = ^ size_t (void){ return(get_focused_window_id());                                   },
    .validate_capture_id    = ^ bool (size_t window_id){ return((window_id > 0 && window_id < 99999) ? true : false); },
...
size_t validate_id_or_get_default_capture_id(enum capture_type_id_t capture_type_id, size_t capture_id){
  return((validate_capture_type_id(capture_type_id, capture_id) == true) ? capture_id : get_default_capture_type_id(capture_type_id));
}

size_t get_default_capture_type_id(enum capture_type_id_t capture_type_id){
  return(capture_types[capture_type_id].get_default_capture_id());
}

bool validate_capture_type_id(enum capture_type_id_t capture_type_id, size_t capture_id){
  return(capture_types[capture_type_id].validate_capture_id(capture_id));
}

CGImageRef capture_type_id_or_default_capture_type_id(enum capture_type_id_t capture_type_id, size_t capture_id){
  return(capture(capture_type_id, validate_id_or_get_default_capture_id(capture_type_id, capture_id)));
}
