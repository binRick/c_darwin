...
#include "libnsgif/include/nsgif.h"
#include "log/log.h"
#include "ms/ms.h"
#include "nsgif-test/nsgif-test.h"
#include "libnsgif/include/nsgif.h"
#include "libnsgif/test/cli.c"
#include "timestamp/timestamp.h"
#define GIF "/tmp/window-1646-1664940333553.gif"
#define PPM_FILE "/tmp/window-1646-1664940333553.ppm"
#define MAX_ARGS 32
...
static struct argv_t {
  const int argc;
  const char *argv[MAX_ARGS];
} const *argvs[ARGV_TYPES_QTY] = {
 [ARGV_TYPE_HELP] = &(struct argv_t){
   .argc = 2, 
   .argv = { 
     "nsgif", "--help", NULL, 
   }, 
 },
 [ARGV_TYPE_GIF] = &(struct argv_t){
   .argc = 3, 
   .argv = { 
     "nsgif", GIF, "--info", NULL 
   }, 
 },
 [ARGV_TYPE_PPM] = &(struct argv_t){
   .argc = 4, 
   .argv = { 
     "nsgif", GIF, "--ppm", PPM_FILE, NULL 
   }, 
 },
 [ARGV_TYPE_LOOPS] = &(struct argv_t){
   .argc = 4, 
   .argv = { 
     "nsgif", GIF, "--loops", "5", NULL 
   }, 
 },
 [ARGV_TYPE_PALETTE] = &(struct argv_t){
   .argc = 3, 
   .argv = { 
     "nsgif", GIF, "--palette", NULL 
   }, 
 },
};
...
TEST t_nsgif(const int argc, const char *argv[]){
...
SUITE(s_nsgif_help) {
  RUN_TESTp(t_nsgif, argvs[ARGV_TYPE_HELP]->argc, argvs[ARGV_TYPE_HELP]->argv);
}
SUITE(s_nsgif_info) {
  RUN_TESTp(t_nsgif, argvs[ARGV_TYPE_GIF]->argc, argvs[ARGV_TYPE_GIF]->argv);
}
SUITE(s_nsgif_ppm) {
  RUN_TESTp(t_nsgif, argvs[ARGV_TYPE_PPM]->argc, argvs[ARGV_TYPE_PPM]->argv);
}
SUITE(s_nsgif_loops) {
  RUN_TESTp(t_nsgif, argvs[ARGV_TYPE_LOOPS]->argc, argvs[ARGV_TYPE_LOOPS]->argv);
}
SUITE(s_nsgif_palette) {
  RUN_TESTp(t_nsgif, argvs[ARGV_TYPE_PALETTE]->argc, argvs[ARGV_TYPE_PALETTE]->argv);
}
...
int main(int argc, char **argv) {
  GREATEST_MAIN_BEGIN();
  RUN_SUITE(s_nsgif_help);
  RUN_SUITE(s_nsgif_info);
  RUN_SUITE(s_nsgif_ppm);
  RUN_SUITE(s_nsgif_loops);
  RUN_SUITE(s_nsgif_palette);
  GREATEST_MAIN_END();
}
