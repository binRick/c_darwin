...
const struct capture_request_t __REQUEST_WRITE_FILE_RETURN_RESULT = {
  .mode          = CAPTURE_REQUEST_MODE_WRITE_FILE_RETURN_RESULT,
...
TEST t_return_resize_factor_write_file(void *REQ){
  ...
  }
  {
    ...
    ASSERT_EQm("captured width too small", res->data->captured_width, 100);
    ASSERT_EQm("captured height too small", res->data->captured_height, 100);
...
TEST t_return_write_file(void *REQ){
  struct capture_request_t *req = (struct capture_request_t *)REQ;
  struct capture_result_t  *res;
  {
    req->window_id = atoi(getenv("WINDOWID"));
    asprintf(&req->file_path, "%swindow-%d-test-return-result-%lu.png", gettempdir(), req->window_id, (size_t)timestamp());
  }
  {
    res = request_window_capture(req);
  }
  ...
  ASSERT_EQm("captured width too small", res->data->captured_width, 100);
  ASSERT_EQm("captured height too small", res->data->captured_height, 100);
  ASSERT_EQm("invalid resized height", res->data->captured_height, res->data->resized_height);
  ASSERT_EQm("invalid resized width", res->data->captured_width, res->data->resized_width);
  ASSERT_GTm("requested file path too small", fsio_file_size(req->file_path), 128);
  PASS();
}

SUITE(s_test_callback){
  //RUN_TEST(t_callback_write_file);
  // RUN_TEST(t_callback_pixels);
}
SUITE(s_test_return){
//  RUN_TESTp(t_return_write_file,(void*)__REQUEST_WRITE_FILE_RETURN_RESULT);
